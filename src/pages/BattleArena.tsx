...
const BattleArena = () => {
  ...
  const handleSubmitCode = async () => {
    if (!battleId || !user || !problem) {
      ...
      console.error("Missing required data:", { battleId, user, problem });
      return;
    }
    ...
    console.log("Starting submission with data:", {
      battle_id: battleId,
      user_id: user.id,
      code: code.length > 20 ? code.substring(0, 20) + "..." : code, // Truncated for logging
      language
    });
    ...
    try {
      ...
      console.log("Battle check result:", battleCheck);
      ...
      if (battleCheck.creator_id !== user.id && battleCheck.defender_id !== user.id) {
        console.error('User is not a participant in this battle:', {
          creator_id: battleCheck.creator_id,
          defender_id: battleCheck.defender_id,
          user_id: user.id
        });
      ...
      // Insert the submission with required fields only
      ...
      if (submissionError || !submission) {
        ...
        toast.error('Failed to submit code: ' + (submissionError?.message || 'Unknown error'));
        setIsSubmitting(false);
        return;
      }
      ...
      try {
        ...
        console.log("API response status:", gptRes.status);
        
        if (!gptRes.ok) {
          const errorText = await gptRes.text();
          console.error("GPT API error:", gptRes.status, errorText);
          
          // Even if API call fails, update the submission status to avoid hanging
          const { error: updateError } = await supabase
            .from('submissions')
            .update({
              status: 'error',
              feedback: `API error: ${gptRes.status} - ${errorText.substring(0, 200)}...`,
              evaluated_at: new Date().toISOString(),
            })
            .eq('id', submission.id);
            
          if (updateError) {
            console.error('Error update failure:', updateError);
          }
        ...
        if (!gptData.choices || !gptData.choices[0]) {
          console.error("Invalid API response structure:", gptData);
          throw new Error("Invalid API response");
        }
        
        const responseText = gptData.choices[0].message.content;
        // More robust score regex pattern
        const scoreMatch = responseText.match(/(\d+)(?:\s*\/\s*100|\s*out of\s*100|\s*points|\s*%)/i);
        const score = scoreMatch ? parseInt(scoreMatch[1]) : 50; // Default to 50 if no valid score found
  
        console.log("Extracted score:", score, "from:", responseText.substring(0, 50) + "...");
        ...
        if (updateError) {
          ...
          if (fetchError || !updatedSubmission) {
            ...
            toast.error('Error retrieving updated submission: ' + (fetchError?.message || 'Unknown error'));
          } else {
            console.log("Retrieved updated submission:", updatedSubmission);
